# use this if Evgen and jzcapa are in different root files, otherwise use MakePickleFile.py
...
def combine_file(pos_path_evgen,pos_path_jzcapa,eventNumber):
    try:
        particle = get_df(pos_path_evgen, treeName= "Particle", branch = ["nNeutrons","avgX","avgY","RP_true_value","RP_gen_value","pt_nuclear"])
        analysis = get_df(pos_path_jzcapa, treeName = "AnalysisTree", branch = ["rpd*Peak_max"])
...
def combine_file_in_folder(side,start_run,end_run):
    output = None
    start = time.time()
    folder_path_evgen = "/projects/engrit/jzcapa/Users/Mike/condorJobs/ToyFermi_qqFibers_LHC_noPedNoise/Output/"
    folder_path_jzcapa = "/projects/engrit/jzcapa/Users/Mike/condorJobs/ToyFermi_qqFibers_LHC_noPedNoise/jzcapa_output/"
    ...
    for i in range(start_run, end_run + 1):
        ...
        path_evgen = folder_path_evgen+file
        path_jzcapa = folder_path_jzcapa+file
        tmp = combine_file(path_evgen,path_jzcapa,i)
        if tmp is None:
            err += 1
